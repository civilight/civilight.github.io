...
const EnemiesData = await GetEnemiesData()

export async function load({ params }) {
	// biome-ignore lint/style/noNonNullAssertion: infallible
	const data = Object.values(EnemiesData.enemies).find((v) => v.enemyId === params.enemyId)!

	const name = GetRegionalString(data.name, params.lang)
	const description = GetRegionalString(data.description, params.lang)

	const iconUrl = EnemiesData.availableIcons.includes(params.enemyId)
	...
	for (const type of Object.values(data.enemyTypes)) {
		// biome-ignore lint/style/noNonNullAssertion: infallible
		const typeData = EnemiesData.enemyTypes.find((v) => v.id === type)!
...
export async function entries() {
	...
	for (const lang of SERVERS) {
		for (const enemy of Object.values(EnemiesData.enemies)) {
